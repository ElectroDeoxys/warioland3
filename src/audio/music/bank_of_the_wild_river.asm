Music_BankOfTheWildRiver_Ch1:
	semitone_offset 12
	tempo 65
	wave WAVE_48
	volume 75
	audio_wait_96
	audio_wait_96

.loop
	wave WAVE_52
	note NOTE_DURATION_12, G_3, 19
	audio_wait_18
	note NOTE_DURATION_6
	audio_wait_6
	note NOTE_DURATION_3, F_3
	audio_wait_12
	note D_3
	audio_wait_12
	note G_3
	audio_wait_12
	note NOTE_DURATION_3
	audio_wait_12
	note NOTE_DURATION_3, F_3
	audio_wait_12
	note D_3
	audio_wait_12
	note G_3
	audio_wait_96
	note NOTE_DURATION_18
	audio_wait_18
	note NOTE_DURATION_6, A_3
	audio_wait_6
	note NOTE_DURATION_3, B_3
	audio_wait_12
	note D_4
	audio_wait_12
	note C_4
	audio_wait_12
	note A_3
	audio_wait_12
	note F_3
	audio_wait_12
	note D_3
	audio_wait_12
	note G_3
	audio_wait_96
	note NOTE_DURATION_6
	audio_wait_6
	note NOTE_DURATION_3, G_4
	audio_wait_12
	note G_3
	audio_wait_6
	note F_4
	audio_wait_12
	note D_4
	audio_wait_12
	note NOTE_DURATION_6, G_3
	audio_wait_6
	note NOTE_DURATION_3, G_4
	audio_wait_12
	note G_3
	audio_wait_6
	note F_4
	audio_wait_12
	note D_4
	audio_wait_12
	note G_4
	audio_wait_96
	note G_3
	audio_wait_12
	note NOTE_DURATION_6
	audio_wait_6
	note NOTE_DURATION_6, A_3
	audio_wait_6
	note NOTE_DURATION_3, B_3
	audio_wait_12
	note D_4
	audio_wait_12
	note C_4
	audio_wait_12
	note A_3
	audio_wait_12
	note F_3
	audio_wait_12
	note D_3
	audio_wait_12
	note G_3
	audio_wait_96
	wave WAVE_50
	note NOTE_DURATION_18, F#3, 19
	audio_wait_18
	note NOTE_DURATION_3, G_3
	audio_wait_6
	note E_3
	audio_wait_12
	note C_3
	audio_wait_12
	note G_3
	audio_wait_48
	note NOTE_DURATION_18, F#3
	audio_wait_18
	note NOTE_DURATION_3, G_3
	audio_wait_6
	note E_3
	audio_wait_12
	note C_3
	audio_wait_12
	note G_3
	audio_wait_12
	note NOTE_DURATION_3
	audio_wait_36
	note NOTE_DURATION_18, C#3
	audio_wait_18
	note NOTE_DURATION_3, D_3
	audio_wait_6
	note B_2
	audio_wait_12
	note G_2
	audio_wait_12
	note D_3
	audio_wait_48
	note NOTE_DURATION_18, C#3
	audio_wait_18
	note NOTE_DURATION_6, D_3
	audio_wait_6
	note NOTE_DURATION_3, B_2
	audio_wait_12
	note G_2
	audio_wait_12
	note NOTE_DURATION_6, D_3
	audio_wait_6
	note C#3
	audio_wait_6
	note NOTE_DURATION_3, D_3
	audio_wait_36
	note NOTE_DURATION_18, F#3
	audio_wait_18
	note NOTE_DURATION_3, G_3
	audio_wait_6
	note E_3
	audio_wait_12
	note C_3
	audio_wait_12
	note NOTE_DURATION_6, G_3
	audio_wait_6
	note F#3
	audio_wait_6
	note NOTE_DURATION_3, G_3
	audio_wait_36
	note NOTE_DURATION_18, A_2
	audio_wait_18
	note NOTE_DURATION_3, G#2
	audio_wait_6
	note A_2
	audio_wait_12
	note C#3
	audio_wait_12
	note E_3
	audio_wait_12
	note G_3
	audio_wait_12
	note F#3
	audio_wait_12
	note E_3
	audio_wait_12
	note NOTE_DURATION_18, D_3
	audio_wait_18
	note NOTE_DURATION_3, D_4
	audio_wait_6
	note C_4
	audio_wait_12
	note A_3
	audio_wait_12
	note NOTE_DURATION_6, D_3
	audio_wait_6
	note NOTE_DURATION_3, D_4
	audio_wait_12
	note NOTE_DURATION_6, D_3
	audio_wait_6
	note NOTE_DURATION_3, C_4
	audio_wait_12
	note A_3
	audio_wait_12
	note D_4
	audio_wait_96
	audio_wait_96
	audio_wait_96
	wave WAVE_51
	audio_wait_12
	note NOTE_DURATION_6, D_2, 19
	audio_wait_6
	note G_2
	audio_wait_6
	note NOTE_DURATION_3, B_2
	audio_wait_24
	note C_3
	audio_wait_12
	note A_2
	audio_wait_12
	note F_2
	audio_wait_12
	note A_2
	audio_wait_12
	audio_wait_12
	note NOTE_DURATION_6, G_2
	audio_wait_6
	note B_2
	audio_wait_6
	note NOTE_DURATION_3, D_3
	audio_wait_24
	note F_3
	audio_wait_12
	note C_3
	audio_wait_12
	note A_2
	audio_wait_12
	note F_2
	audio_wait_12
	audio_jump .loop
.end
	wave WAVE_52
	note NOTE_DURATION_12, G_3, 19
	audio_wait_18
	note NOTE_DURATION_6
	audio_wait_6
	note NOTE_DURATION_3, F_3
	audio_wait_12
	note D_3
	audio_wait_12
	note G_3
	audio_wait_12
	note NOTE_DURATION_3
	audio_wait_12
	note NOTE_DURATION_3, F_3
	audio_wait_12
	note D_3
	audio_wait_3
	audio_end

Music_BankOfTheWildRiver_Ch2:
	semitone_offset 0
	wave WAVE_51
	volume 66
	audio_wait_12
	note NOTE_DURATION_6, G_2, 19
	audio_wait_6
	note B_2
	audio_wait_6
	note NOTE_DURATION_3, D_3
	audio_wait_24
	note F_3
	audio_wait_12
	note C_3
	audio_wait_12
	note A_2
	audio_wait_12
	note C_3
	audio_wait_12
.sub1
	audio_wait_12
	note NOTE_DURATION_6, B_2, 19
	audio_wait_6
	note D_3
	audio_wait_6
	note NOTE_DURATION_3, G_3
	audio_wait_24
	note A_3
	audio_wait_12
	note F_3
	audio_wait_12
	note C_3
	audio_wait_12
	note F_3
	audio_wait_12
	audio_ret

.loop
.sub2
	wave WAVE_47
	audio_wait_12
	note NOTE_DURATION_6, G_2, 19
	audio_wait_6
	note B_2
	audio_wait_6
	note NOTE_DURATION_3, D_3
	audio_wait_24
	note F_3
	audio_wait_12
	note C_3
	audio_wait_12
	note A_2
	audio_wait_12
	note C_3
	audio_wait_12
	audio_ret

	audio_call .sub2
	audio_call .sub2
	audio_wait_12
	note NOTE_DURATION_6, B_2, 19
	audio_wait_6
	note D_3
	audio_wait_6
	note NOTE_DURATION_3, G_3
	audio_wait_24
	note A_3
	audio_wait_12
	note F_3
	audio_wait_12
	note C_3
	audio_wait_12
	note A_2
	audio_wait_12
	audio_call .sub2
	audio_call .sub2
	audio_call .sub2
.sub3
	audio_wait_12
	note NOTE_DURATION_6, G_2, 19
	audio_wait_6
	note B_2
	audio_wait_6
	note NOTE_DURATION_3, D_3
	audio_wait_24
	note G_3
	audio_wait_12
	note D_3
	audio_wait_12
	note B_2
	audio_wait_12
	note D_3
	audio_wait_12
	audio_ret

.sub4
	audio_wait_12
	note NOTE_DURATION_6, C_3, 19
	audio_wait_6
	note E_3
	audio_wait_6
	note NOTE_DURATION_3, G_3
	audio_wait_24
	note C_4
	audio_wait_12
	note G_3
	audio_wait_12
	note E_3
	audio_wait_12
	note G_3
	audio_wait_12
	audio_ret

	audio_call .sub4
	audio_call .sub3
	audio_call .sub3
	audio_call .sub4
	audio_wait_12
	note NOTE_DURATION_6, A_2, 19
	audio_wait_6
	note C#3
	audio_wait_6
	note NOTE_DURATION_3, E_3
	audio_wait_24
	note A_3
	audio_wait_12
	note E_3
	audio_wait_12
	note C#3
	audio_wait_12
	note E_3
	audio_wait_12
	audio_wait_12
	note NOTE_DURATION_6, D_3
	audio_wait_6
	note F#3
	audio_wait_6
	note NOTE_DURATION_3, A_3
	audio_wait_24
	note D_4
	audio_wait_12
	note A_3
	audio_wait_12
	note F#3
	audio_wait_12
	note A_3
	audio_wait_12
	audio_wait_12
	note NOTE_DURATION_6, D_3
	audio_wait_6
	note F#3
	audio_wait_6
	note NOTE_DURATION_3, A_3
	audio_wait_24
	note D_4
	audio_wait_12
	note F#2
	audio_wait_12
	note G_2
	audio_wait_12
	note A_2
	audio_wait_12
	audio_call .sub2
	audio_call .sub1
	audio_call .sub1
	audio_wait_12
	note NOTE_DURATION_6, D_3, 19
	audio_wait_6
	note G_3
	audio_wait_6
	note NOTE_DURATION_3, B_3
	audio_wait_24
	note C_4
	audio_wait_12
	note A_3
	audio_wait_12
	note F_3
	audio_wait_12
	note C_3
	audio_wait_12
	audio_jump .loop
.end
	audio_wait_12
	note NOTE_DURATION_6, G_2, 19
	audio_wait_6
	note B_2
	audio_wait_6
	note NOTE_DURATION_3, D_3
	audio_wait_24
	note F_3
	audio_wait_12
	note C_3
	audio_wait_12
	note A_2
	audio_wait_12
	note F_2
	audio_wait_3
	audio_end

Music_BankOfTheWildRiver_Ch3:
	semitone_offset 0
	wave WAVE_61
	volume 108
	note NOTE_DURATION_3, G_1, 24
	audio_wait_36
	note D_1
	audio_wait_36
	note NOTE_DURATION_3
	audio_wait_24
.sub1
	note NOTE_DURATION_3, G_1, 24
	audio_wait_36
	note D_1
	audio_wait_36
	note NOTE_DURATION_3
	audio_wait_24
	audio_ret

.loop
	audio_call .sub1
	audio_call .sub1
	audio_call .sub1
.sub2
	note NOTE_DURATION_3, G_1, 24
	audio_wait_36
	note D_1
	audio_wait_24
	note NOTE_DURATION_3
	audio_wait_6
	note NOTE_DURATION_3, E_1
	audio_wait_6
	note F_1
	audio_wait_6
	note NOTE_DURATION_3, E_1
	audio_wait_6
	note NOTE_DURATION_3, D_1
	audio_wait_12
	audio_ret

	audio_call .sub1
	audio_call .sub1
	audio_call .sub1
.sub3
	note NOTE_DURATION_3, G_1, 24
	audio_wait_36
	note D_1
	audio_wait_24
	note G_1
	audio_wait_12
	note A_1
	audio_wait_12
	note B_1
	audio_wait_12
	audio_ret

.sub4
	note NOTE_DURATION_3, C_2, 24
	audio_wait_36
	note G_1
	audio_wait_36
	note NOTE_DURATION_3
	audio_wait_24
	audio_ret

	audio_call .sub4
	audio_call .sub1
	audio_call .sub3
	note NOTE_DURATION_3, C_2, 24
	audio_wait_36
	note G_1
	audio_wait_36
	note C_2
	audio_wait_24
	note A_1
	audio_wait_36
	note E_1
	audio_wait_36
	note A_1
	audio_wait_24
	note D_1
	audio_wait_36
	note A_1
	audio_wait_36
	note D_1
	audio_wait_24
	note NOTE_DURATION_3
	audio_wait_36
	note NOTE_DURATION_3, A_1
	audio_wait_24
	note D_1
	audio_wait_12
	note E_1
	audio_wait_12
	note F#1
	audio_wait_12
	audio_call .sub1
	audio_call .sub1
	audio_call .sub1
	audio_call .sub2
	audio_jump .loop
.end
	note NOTE_DURATION_3, G_1, 24
	audio_wait_36
	note D_1
	audio_wait_36
	note NOTE_DURATION_3
	audio_wait_3
	audio_end

Music_BankOfTheWildRiver_Ch4:
	semitone_offset 0
	wave WAVE_28
	volume 85
	note NOTE_DURATION_3, F#0, 23
	audio_wait_12
	note G#0
	audio_wait_6
	note F#0
	audio_wait_6
	note G#0
	audio_wait_12
	note F#0
	audio_wait_12
	note G#0
	audio_wait_12
	note NOTE_DURATION_3
	audio_wait_12
	note NOTE_DURATION_3, F#0
	audio_wait_12
	note G#0
	audio_wait_12
.sub1
	note NOTE_DURATION_3, F#0, 23
	audio_wait_12
	note G#0
	audio_wait_6
	note F#0
	audio_wait_6
	note G#0
	audio_wait_12
	note F#0
	audio_wait_12
	note G#0
	audio_wait_12
	note NOTE_DURATION_3
	audio_wait_12
	note NOTE_DURATION_3, F#0
	audio_wait_12
	note G#0
	audio_wait_12
	audio_ret

.loop
	audio_call .sub1
	audio_call .sub1
	audio_call .sub1
	audio_call .sub1
	audio_call .sub1
	audio_call .sub1
	audio_call .sub1
	audio_call .sub1
	audio_call .sub1
	audio_call .sub1
	audio_call .sub1
	audio_call .sub1
	audio_call .sub1
	audio_call .sub1
	audio_call .sub1
	audio_call .sub1
	audio_call .sub1
	audio_call .sub1
	audio_call .sub1
	audio_call .sub1
	audio_jump .loop
.end
	note NOTE_DURATION_3, F#0, 23
	audio_wait_12
	note G#0
	audio_wait_6
	note F#0
	audio_wait_6
	note G#0
	audio_wait_12
	note F#0
	audio_wait_12
	note G#0
	audio_wait_12
	note NOTE_DURATION_3
	audio_wait_12
	note NOTE_DURATION_3, F#0
	audio_wait_12
	note G#0
	audio_wait_3
	audio_end

Music_BankOfTheWildRiver:
	db $4, $2
	dw Music_BankOfTheWildRiver_Ch1
	dw Music_BankOfTheWildRiver_Ch2
	dw Music_BankOfTheWildRiver_Ch3
	dw Music_BankOfTheWildRiver_Ch4

; unused?
	dw Music_BankOfTheWildRiver_Ch1.loop
	dw Music_BankOfTheWildRiver_Ch2.loop
	dw Music_BankOfTheWildRiver_Ch3.loop
	dw Music_BankOfTheWildRiver_Ch4.loop

	dw Music_BankOfTheWildRiver_Ch1.end
	dw Music_BankOfTheWildRiver_Ch2.end
	dw Music_BankOfTheWildRiver_Ch3.end
	dw Music_BankOfTheWildRiver_Ch4.end
