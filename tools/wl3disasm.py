...
unconditional_returns = [0xc9, 0xd9, 0xe7] # e7 begins a script, which is not handled by wl3disasm
absolute_jumps = [0xc3, 0xc2, 0xca, 0xd2, 0xda]
call_commands = [0xcd, 0xc4, 0xcc, 0xd4, 0xdc, 0xdf]
relative_jumps = [0x18, 0x20, 0x28, 0x30, 0x38]
unconditional_jumps = [0xc3, 0x18, 0xe9, 0xef]
...
def check_farcall(ops):
	if len(ops) != 15:
		raise ValueError("Wrong number of bytes to check for farcall")

	if ops[0] != 0x3e: # 'ld a, x'
		return False
	if ops[2] != 0xe0 or ops[3] != 0x85: # 'ldh bank'
		return False
	if ops[4] != 0x3e: # 'ld a, x'
		return False
	if ops[6] != 0xe0 or ops[7] != 0x8d: # 'ldh low ptr'
		return False
	if ops[8] != 0x3e: # 'ld a, x'
		return False
	if ops[10] != 0xe0 or ops[11] != 0x8e: # 'ldh high ptr'
		return False
	if ops[12] != 0xcd or ops[13] != 0x80 or ops[14] != 0xff: # 'Farcall'
		return False

	if ops[9] < 0x40 or ops[9] >= 0x80:
		return False
...
def check_update_anim(ops):
	...
	return (ops[0] == 0x3e  and ops[2] == 0xe0 \
	...
	def output_bank_opcodes(self, start_offset, stop_offset, hard_stop=False, parse_data=False, include_last_address=True):
		...
		while True:
			...
			if not is_data or not parse_data:
				...
				elif opcode_nargs == 1 and not is_farcall:
				...
				# overwrite the arguments for farcall
					opcode_arg_1 = rom[offset+1] # bank
					opcode_arg_2 = rom[offset+5] # low ptr
					opcode_arg_3 = rom[offset+9] # high ptr
					...
					# format the label that was created into the opcode string
					opcode_output_str = 'farcall ' + target_label
				else:
					# error checking
				...
				# farcalls and update_anim_1 are 15 bytes long
...
if __name__ == "__main__":
	...
	ap.add_argument("-o", dest="filename", default="wl3disasm_output.asm")
	ap.add_argument("-s", dest="symfile", default="warioland3.sym")
